package com.br.accava.acelerador.backend.jab;
...
import com.br.accava.acelerador.backend.modelo.AtributoMetodoModel;
import com.br.accava.acelerador.backend.modelo.ChamadaModel;
import com.br.accava.acelerador.backend.modelo.ClsModel;
import com.br.accava.acelerador.backend.modelo.MetodoModel;

/**@Complexidade 5/5*/
public class ConversorJab {
	...
	/*------------------------------- IDENTIFICADORES PARAMETROS DE PROC --------------------------------*/

	
	/*------------------------------------ CONTADORES TIPOS DE PROCS ------------------------------------*/
	private static int contadorProcDeclaradaPorAtribuicao = 0;
	
	
	/*---------------------------------------------- Configuração para detectar Procs -------------------------------------------*/
	...
	/*-------------------------------------------- Configuracao para detectar funcoes  -------------------------------------------*/
	...
	private static ChamadaModel chamadaModelo = new ChamadaModel();
	private static List<ChamadaModel> listaChamadaModelo =  new ArrayList<ChamadaModel>();
	private static String atributosChamada = "";
...
//		iniciaLeituraArquivoCls(new File(ARQUIVO_SOLTO));
	...
	public static void buscaArquivos(File pasta) throws Exception {
		for (File arquivo : pasta.listFiles()) {
			if(arquivo.isDirectory()) {
				buscaArquivos(arquivo);
	...
	private static void limpaUltimaEstruturaGerada() {
		...
		chamadaModelo = new ChamadaModel();
		listaChamadaModelo =  new ArrayList<ChamadaModel>();
		atributosChamada = "";
		...
		if (siglaComProcAvulsa) {
			...
			if (l.contains(IDENTIFICADOR_PROC_AVULSA) && contadorProcAvulsa <= 0) {
				...
				if (nomeChamada.contains("\"")) {
					...
					chamadaModelo.setNomeChamada(nomeChamada);
					chamadaModelo.setTipoChamada("PROC AVULSA");
					chamadaModelo.setAssinaturaChamada(l);
			...
			if (contadorProcAvulsa > 0) {
				if (l.contains(IDENTIFICADOR_ATRR_PROC_POR_CREATEPARAMETER)) {
					...
					System.out.println("\t\t\t ->" + atributosChamada);
				}
			}
			
			if (contadorProcAvulsa > 0 && l.contains(IDENTIFICADOR_TERMINO_PROC_POR_ACTIVECON)) {
				String[] split = atributosChamada.split(",");
				listaChamadaModelo.add(new ChamadaModel(chamadaModelo.getNomeChamada(), chamadaModelo.getAssinaturaChamada(), chamadaModelo.getTipoChamada(), split));
				atributosChamada = "";
				chamadaModelo = new ChamadaModel();
				contadorProcAvulsa--;
			}
		...
		if (siglaComChamadaProc) {
			...
			/*(ii) Proc por Atribuicao - LINHA INICIAL*/
				...
				if (auxiliador.contains("\"") ) {
					...
					if (auxiliador.contains(IDENTIFICADOR_PROC_DECLARADA)) {
						...
						chamadaModelo.setNomeChamada(auxiliador.trim());
						chamadaModelo.setAssinaturaChamada(l);
						chamadaModelo.setTipoChamada("PROC DECLARADA POR ATRIBUICAO");
			...
			/*(i) PROC por Atribuição - Linhas dos Parametros*/
				...
				if (aux.contains("_")) {
					...
					atributosChamada = atributosChamada + aux;
					...
					atributosChamada = atributosChamada + aux;
					String recorteAtrrProcMesmaLinha = recorteAtrrProcMesmaLinha(atributosChamada);
	...
	private static void inicializacaoProcsPorCall(String l) {
		...
		if (l.contains(IDENTIFICADOR_PROC_DECLARADA_POR_CALL_TERMINO)) {
			if (l.contains("(")) {
				...
				listaChamadaModelo.add(new ChamadaModel(aux, l.trim(), "PROC DECLARADA POR CALL MONO LINHA", split));
		...
		/*Utilizacao de Procs por Call -> Inicio e fim em linhas separadas*/
			...
			if (l.contains("(") && contadorProcDeclaradaPorAtribuicao <= 0) {
				...
				chamadaModelo.setNomeChamada(nomeProc);
				chamadaModelo.setAssinaturaChamada(l);
				chamadaModelo.setTipoChamada("PROC DECLARADA POR CALL MULTI LINHA");
	...
	private static List<ChamadaModel> popularChamadaModelo(List<ChamadaModel> listCM) {
		...
		for(ChamadaModel cm : listaChamadaModelo) {
			listCM.add(cm);
		}
		return listCM;
	}
	...
	private static void recortaNomePrimeiroIdentificadorProc(String l) {
		...
		atributosChamada =  atributosChamada + nome +",";
	}
	...
	private static void imprimirEstrutura() {
